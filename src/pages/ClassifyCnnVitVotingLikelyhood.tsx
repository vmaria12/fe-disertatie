...
import { Upload, Loader, ArrowLeft, Activity, Vote, CheckCircle2, AlertTriangle } from 'lucide-react';

interface ClassifyCnnVitVotingLikelyhoodProps {
...
export function ClassifyCnnVitVotingLikelyhood({ onNavigate }: ClassifyCnnVitVotingLikelyhoodProps) {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:8000/api/detect-tumor/neuronal-network/voting-likelihood', {
                method: 'POST',
                body: formData,
            });
            ...
            <div className="max-w-6xl mx-auto px-6">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-8">
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <h2 className="text-3xl font-bold text-slate-900">Votare - Suma Probabilităților (CNN & ViT)</h2>
                    </div>
                    ...
                    <div className="grid lg:grid-cols-2 gap-8">
                        ...
                        <div>
                            <div className="mb-6">
                                ...
                                <div className="border-2 border-dashed border-teal-300 rounded-xl p-8 text-center hover:border-teal-500 transition-colors cursor-pointer bg-teal-50">
                                    <input
                                        type="file"
                                        accept="image/*"
                            ...
                            {uploadedImage && (
                                <div className="space-y-3">
                                    <button
                                        ...
                                        className="w-full px-6 py-3 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 transition-colors disabled:bg-slate-400 flex items-center justify-center gap-2"
                                    >
                                        {isAnalyzing ? (
                                            <>
                                ...
                                <div className="space-y-6">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-2">
                                                    <span className="font-medium text-slate-700">Scor Total (Sumă):</span>
                                                    <span className="text-xl font-bold font-mono text-slate-900">
                                                        {result.voting_result.total_score_sum}
                                                    </span>
                                                </div>
                                            </div>
                                            {result.voting_result.winning_class !== 'notumor' ? (
                                    ...
                                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                        <h4 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                                            ...
                                            Sumele Probabilităților (Calcul Detaliat)
                                        </h4>
                                        <div className="space-y-3">
                                            {Object.entries(result.voting_result.all_scores_sum).map(([className, score]) => (
                                                <div key={className} className={`flex items-center justify-between p-3 rounded-lg transition-colors ${className === result.voting_result.winning_class ? 'bg-teal-50 border border-teal-200' : 'bg-slate-50'
                                                    }`}>
                                                    <span className={`font-medium capitalize ${className === result.voting_result.winning_class ? 'text-teal-900' : 'text-slate-700'}`}>
                                                        {className}
                                                    </span>
                                                    <span className={`font-mono font-bold ${className === result.voting_result.winning_class ? 'text-teal-700' : 'text-slate-600'}`}>
                                                        {score}
                                                    </span>
                                                </div>
                                            ))}
                                    ...
                                    {/* Individual Model Results (Optional but helpful) */}
